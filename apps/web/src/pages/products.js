...
const ProductsPage = () => {
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "space-y-2", children: [_jsx("h1", { className: "text-2xl font-semibold text-slate-900 dark:text-white", children: "Browse products" }), _jsx("p", { className: "text-sm text-slate-600 dark:text-slate-300", children: "Every interaction helps tailor your feed. View, like, or purchase to refine the collaborative filtering model." })] }), isLoading ? (skeleton) : (_jsx(ProductGrid, { products: (data?.items || []).map((item) => ({
                    ...
                    price: item.price,
                    image: item.images?.[0]
                })), onView: (id) => handleInteraction(id, 'view'), renderFooter: (product) => (_jsx(InteractionButtons, { liked: favorites.isFavorite(product.id), inCart: cart.items.some((item) => item.product._id === product.id), onToggleLike: () => handleFavorite(product.id), onAddToCart: () => handleAddToCart(product.id) })) }))] }));
};
